...
- **Operator** — day-to-day invoice work. Can upload invoices, review/edit/confirm/reprocess invoices, manage companies (list, view, delete, merge), run exports, edit their own profile/password, and manage VAT classifiers (create, edit, delete, import from catalog). Operators cannot open the LLM/Organization/MCP/Webhooks settings pages (the tabs are hidden and the routes redirect to `/invoices`), and they see export templates **read-only** (no New/Edit/Delete/Favorite buttons).
...
Instead of a plain dropdown, the New/Edit user form shows two selectable **role cards** — Operator and Administrator. Each card lists what that role can do (e.g. operator: upload/review/confirm/export invoices, manage companies, edit own profile, manage VAT classifiers, read-only export templates; administrator: everything an operator can do plus LLM/organization/MCP settings, export templates, and user management). Clicking a card selects the radio inside it; the selected card is highlighted with the primary border/box-shadow. The submitted value is read server-side exactly like the previous select, so existing validation, last-admin protection, and forbidden self role-change still apply.

### 66d. I'm an operator and tried to open a settings page — what happened?
Operators can open the **Settings** page from the navbar menu. Clicking **Nustatymai / Settings** lands on `/settings`, which redirects operators (HTTP 303) to the first tab they can view — **VAT classifiers** (`/settings/vat-classifiers`). The settings tab bar shows operators only the tabs they can access: **VAT classifiers** (full manage — create, edit, delete, import from catalog) and **Export templates** (read-only — add/edit/delete/favorite actions are hidden and rejected server-side with `403 Forbidden`). The admin-only tabs — **Artificial Intelligence (LLM)**, **Organization**, **AI Agents (MCP)**, and **Users** — are hidden from operators in the tab bar. If an operator reaches an admin-only settings route directly (stale bookmark, typed URL), the server redirects browser GET requests to `/invoices` with an "Admin only" flash and returns `403 Forbidden` for any POST / form submission to those admin-only settings endpoints.
...
Webhook URLs must use **HTTPS** and must not resolve to an internal address. Both webhook URLs and API export template URLs are checked against SSRF (server-side request forgery) protections: the hostname is resolved and any IP that is loopback, private (RFC 1918), link-local, multicast, unspecified, or a known cloud metadata endpoint (e.g. `169.254.169.254`) is rejected. This blocks tricks like `169.254.169.254.nip.io` that resolve to the metadata IP. A second check runs at TCP connect time so DNS rebinding between validation and connect is also blocked. Webhook URLs are validated when you save them in **Settings → Webhooks** (admin-only), not only when an event fires, so a bad URL is rejected immediately.

### 72b. Where do I configure integration webhooks?
Webhooks are configured at the organization level in **Settings → Webhooks** (admin-only tab). An administrator sets one URL per event — **Invoice processed**, **Invoice confirmed**, and **Invoice exported**. When the corresponding event fires for any invoice in the organization, the server POSTs a JSON payload (event type, timestamp, and the canonical export payload) to the configured URL. Because webhooks are org-level, all invoice events in the organization notify the configured endpoints regardless of which user triggered the action. Previously webhooks were per-user and configured on the profile page; they have been moved to Settings because they are an integration/organization concern, not a personal preference. The profile page now only manages your name, email, and password.
